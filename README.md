...
## インフラ構成図
![infrastructure-diagram](https://github.com/kima-s/Notification-Management-API/assets/130459578/132d099c-4f24-4c0c-9cdb-d584858706e1)
...
## 苦労したこと
特にReactでフロント側を実装する際に、コンポーネントとして各機能を切り分けたので、各コンポーネント間で値を共有して利用する実装であったり、ファイル構成、各Hooksの知識の習得には非常に時間がかかりました。レンダリングのタイミングや非同期処理など覚えることも多く難しさを感じました。 

また、AWSでのデプロイの際にも、仕組みの理解や予期せぬエラーの連続で多くの時間を要しました。各ファイルの組み合わせでアプリケーションをデプロイできると思うので、バージョンの違い一つでうまく物事が進まなくなり、難しさを感じました。

苦しみながらではありましたが、最終的に自分でサーバーの起動からアプリケーションの公開まで漕ぎ着けることができたので、大変良い経験になりました。
...
業務として取り組むことになると当然納期があると思います。実装予定の仕様を固めて、現実的な作成計画を立ててから作成開始することがいかに難しく大事であるかを痛感しました。今後、アプリケーション作成など取り組む際には、作業内容の細分化や内容把握、作成タイミングでの自分の知識量の把握に努め、より正確な作成期間を定めた上で取り組むようにしたいです。
